	...
	public boolean isInRole(String userid) {
		...
		synchronized(lock) {
			...
			try {
				...
				PGEabdb2 user = facade.SoapRequestPGEabdb2(env.getCodBanca(), userid, ConstantsSms.MY_APPL_ID);
				String authority = user.getPG_COD_AUTHOR();
				if (! (authority == null)) authority = authority.trim();
				isUserInRole = SingleSecurityProfile.MT.getPgProfileDescription().equals(authority);
