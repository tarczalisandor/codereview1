...
public class GatewaySmsProvider extends GatewayA2Psms {
	...
	public InternalSmsResponse sendSingleMessage(InternalSmsRequest internalSmsRequest, XasUser xasUser, Provider provider, Originator originator, boolean multipart) {
		...
		if (sendOk) {
			...
			internalSmsResponse.setMessage(MessageFormat.format(Constants.XAS00000I_MESSAGE2, new String[]{internalSmsRequest.getUuid(), Integer.toString(internalSmsRequest.getDstByteLength()), "?", internalSmsRequest.getEncodingDescription()}));
		} else {
			internalSmsResponse = new InternalSmsResponse(Constants.XAS00004E_CODE, Constants.XAS00004E_MESSAGE, null, null);
		}
