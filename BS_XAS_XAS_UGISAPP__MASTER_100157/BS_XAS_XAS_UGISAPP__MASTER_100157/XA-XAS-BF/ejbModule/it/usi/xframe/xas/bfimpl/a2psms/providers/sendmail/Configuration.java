...
public class Configuration {
	...
	public static Object load(XMLConfiguration config, String customizationPath) throws XASException {
		...
		if (mailMapArray.length == 0) 
			throw new XASException(Constants.XAS00027E_MESSAGE, null, Constants.XAS00027E_CODE, new Object[] {path});
		for (int oi = 0; oi < mailMapArray.length; oi++) {
			path = customizationPath + "." + TAG_MAILMAP + "(" + oi + ")[@" + ATT_USER + "]";  
			String user = config.getString(path);
			if (user == null) throw new XASException(Constants.XAS00027E_MESSAGE, null, Constants.XAS00027E_CODE, new Object[] {path});

			
			path = customizationPath + "." + TAG_MAILMAP + "(" + oi + ")[@" + ATT_ADDRESS + "]";  
			value = config.getString(path); 
			if (value == null) throw new XASException(Constants.XAS00027E_MESSAGE, null, Constants.XAS00027E_CODE, new Object[] {path});
