	...
	// *** MULTIPART DEFINITION ***
 	...
 	public InternalSmsResponse sendMessage(InternalSmsResponse internalSmsResponse, InternalSmsRequest internalSmsRequest, XasUser xasUser, Provider provider, Originator originator, boolean multipart, Chronometer chronometer) throws XASException {
		...
		if (!multipart) {
        	...
        	mtSendingResult = sendMail(myUUID, xasUser, provider, originator, mtMessage, 1, internalSmsRequest.getMsg(), chronometer);
...
//	    		System.out.println("maxBytesPerPart=" + maxBytesPerPart + ", nByte=" + nByte + ", encodedBytes.length=" + encodedBytes.length + ", byteCount=" + byteCount);
	        	...
	        	mtSendingResult = sendMail(myUUID, xasUser, provider, originator, mtMessage, totParts, internalSmsRequest.getMsg(), chronometer);
 	 ...
 	 * @throws XASException
 	 */
	private MtSendingResult sendMail(String myUUID, XasUser xasUser, Provider provider, Originator originator, MtMessage mtMessage, int totParts, String message, Chronometer chronometer) throws XASException {
		SmartLog sl = new SmartLog(SmartLog.COUPLING_LOOSE_I).logItCompact(Constants.MY_APPL_ID, Constants.MY_LOG_VER, GatewaySmsProvider.class.getName(), myUUID, SmartLog.V_SCOPE_DEBUG)
	...
	sl.preset("myPreset");
		...
		if (mailTo == null) 
			throw new XASException(Constants.XAS00098E_MESSAGE, null, Constants.XAS00098E_CODE, new Object[] {"No mail address defined for " + xasUser.getNameMedium() + " or " + xasUser.getNameSmall(), "N/A"});
		...
		try {
			...
			internalSmsResponse.setCode(Constants.XAS00000I_CODE);
			internalSmsResponse.setMessage(MessageFormat.format(Constants.XAS00000I_MESSAGE2, new String[]{myUUID, Integer.toString(mtMessage.getPayloadUd().length), Integer.toString(totParts), mtMessage.getPayloadUddc().getValue()}));
        	...
        	internalSmsResponse = new InternalSmsResponse(Constants.XAS00098E_CODE, Constants.XAS00098E_MESSAGE, new String[] {errors.toString()}, null);
			mtSendingResult = new MtSendingResult();
		...
		if (Result.KO.equals(mtSendingResult.getResult())) {
			...
			throw new XASException(Constants.XAS00098E_MESSAGE, null, Constants.XAS00098E_CODE, new Object[] {errorMessage, mtSendingResult.getReason()});
		}  
