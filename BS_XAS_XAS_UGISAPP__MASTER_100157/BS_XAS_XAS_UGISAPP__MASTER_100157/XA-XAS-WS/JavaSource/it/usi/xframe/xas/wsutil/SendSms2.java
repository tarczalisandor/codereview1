...
public class SendSms2 {
	...
	public SmsResponse sendSms2(SmsRequest	smsRequest) throws ServiceFactoryException, RemoteException, XASException {
		...
		try{ 
			SmartLog sl = new SmartLog().logItCompact(Constants.MY_APPL_ID, Constants.MY_LOG_VER, SendSms2.class.getName(), Constants.MY_UUID_INIT, SmartLog.V_SCOPE_DEBUG); 
	...
	public SmsResponse3 sendSms3(SmsRequest3 smsRequest) throws RemoteException {
		...
		try{ 
			...
			SmartLog sl = new SmartLog().logItCompact(Constants.MY_APPL_ID, Constants.MY_LOG_VER, SendSms2.class.getName(), myUUID, SmartLog.V_SCOPE_DEBUG)
			.logIt(SmartLog.K_METHOD, "sendSms3", SmartLog.K_PHASE, SmartLog.V_PHASE_ENTER, Constants.K_UUID_TYPE, myUUID).preset("default"); 
